...
contract AMM is AccessControl{
	...
	*/
    ...
    ERC20 sellERC = ERC20(sellToken);
    ERC20 buyERC = ERC20(buyToken);

    uint256 reserveSell = sellERC.balanceOf(address(this));
    uint256 reserveBuy = buyERC.balanceOf(address(this));

    require(sellERC.transferFrom(msg.sender, address(this), sellAmount), "Transfer failed");

    uint256 adjustedSellAmount = (sellAmount * (10000 - feebps)) / 10000;

    uint256 newSellReserve = reserveSell + adjustedSellAmount;
    uint256 expectedBuyReserve = invariant / newSellReserve;
    require(reserveBuy > expectedBuyReserve, "Insufficient output");

    uint256 buyAmount = reserveBuy - expectedBuyReserve;

    require(buyERC.transfer(msg.sender, buyAmount), "Transfer out failed");
